...
export function TagsField<T extends FieldValues>({
  ...
  return (
    <Field.Root required={required} invalid={!!error} w="full" color="gray.700">
                ...
                {field.value?.map((tag: string, index: number) => (
                  ...
                  >
                    <TagsInput.ItemPreview
                      bg="white"
                      color="gray.700"
                      _highlighted={{ filter: "brightness(0.9)", color: "gray.700" }}
                    >
                      <TagsInput.ItemText color='white' bg='white' >{tag}</TagsInput.ItemText>
      ...
      <Field.ErrorText color="gray.700">{error?.message?.toString()}</Field.ErrorText>
    </Field.Root>
  );
}
